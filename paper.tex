...
and build the short-term wind speed forecasting model. Firstly, ICEEMDAN
...
第三章：使用技术介绍。主要讲述ICEEMDAN分解技术、Prophet、GRU网络、
...
通过Python语言netCDF4库\footnote{提取所用代码请见附录部分1}
...
列预测算法框架。其基于累加模型，可以学习年、周和日季节性以及假日效应等非线性
趋势，最适用于具有强烈季节性影响的时间序列，可以通过R和Python实现调
用。Prophet的预测速度快，并且提供了完全自动化的时间序列预测功能，无需手动操
作即可对杂乱的数据进行合理预测，其对异常值、缺失数据和时间序列中的剧烈
