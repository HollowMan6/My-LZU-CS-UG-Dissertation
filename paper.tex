...
The final results show that by using the multi-feature ICEEMDAN-Prophet-GRU-NN
model proposed in this paper, we can significantly improve the accuracy of
short-term wind speed prediction for 12 hours, so it has supremacy.
}}
...
则占比19.6\%，占据着主要地位。\cite{能源数据2021王庆一}
...
在精度上的优越性。Alencar, D. B.等人\cite{alencar2018hybrid}提出了一种基于
...
\cite{rumelhart1986learning}，由于其时序无关，采用了层状传递结构，
...
\footnote{本部分内容参考了Ruder, Sebastian的梯度下降优化算法综述\cite{ruder2016overview}}
\begin{itemize}
    \item[a. ] SGD
\end{itemize}
...
\begin{figure}[H]
	...
	\subfloat[风速ICCEMDAN-GRU]{
        \includegraphics[width=0.36\textwidth]{figures/wind_gru_predict_test.pdf}
    }
	\subfloat[风速ICCEMDAN-Prophet-GRU]{
        \includegraphics[width=0.36\textwidth]{figures/wind_only_prophet_gru_predict_test.pdf}
    }
    \subfloat[多特征ICCEMDAN-Prophet-GRU-NN]{
        \includegraphics[width=0.36\textwidth]{figures/all_feature_prophet_gru_predict_test.pdf}
    }
...
\item 模型基于最新信号分解技术ICEEMDAN（2014年）\cite{colominas2014improved}
，还采用Facebook推出的新型时间序列框架 Prophet（2018年）\cite{taylor2018forecasting}
与最新循环神经网络 GRU 模型（2014年）\cite{cho2014learning}进行组合。
...
\item 对相关模型
进行优化，突破模型的传统，将GELU激活函数（2016年）\cite{hendrycks2016gaussian}
以及Nadam优化器（2016年）\cite{dozat2016incorporating}和Huber损失函数应用到短期
风速预测领域。
