...
with the inspiration provided by the numerical weather prediction method,
...
Prophet分析的时段对应的结果以及分量和原始数据一起进行基于径向基函数（RBF）的核
...
（北纬40.65 度，东经 96.95 度）的2017年1月1日0时整至2018年12月30日21时整
\footnote{此处所述时间均为协调世界时(UTC, Universal Time Coordinated)。}
共5832条数据。生成数据格式为CSV（逗号分隔值，Comma-Separated Values）文件，包
...
以评估12小时的预测性能，为超参数调优提供依据。首先对训练集和验证集进行六四分，共730天的
数据选择前438天（3504条数据）作为初始训练数据。对于后295天（2324条数据），每隔1天对
...
\begin{figure}[H]
    ...
    \caption{多特征ICEEDMAN-Prophet-GRU模型结构图}
    \label{fig_ICCEMDAN_Prophet_GRU_All_Features}
\end{figure}
图\ref{fig_ICCEMDAN_Prophet_GRU_Wind}展示了多特征ICEEDMAN-Prophet-GRU模型结构。
...
的GRU模型预测值（5820，1，7），再将这些预测值通过使用RBF核函数的核主成分分析（KPCA），
...
\begin{figure}[H]
    ...
    \caption{多特征ICEEDMAN-Prophet-GRU模型中使用的NN模型网络结构图}
    \label{fig_gru_nn}
\end{figure}
...
数值在25\%以内的比值）封装为print\_metrics函数调用，相关代码见附录部分A.6。
\begin{itemize}
\item[1. ] 均方误差（MSE）
...
data_list=[]
model_list=[]
for weather_feature_index in range(len(weather_features)-1, len(weather_features)):
...
if os.path.isdir(modelname):
    ...
    model.summary()
    keras.utils.plot_model(model, to_file=modelname+'model_plot.pdf', show_shapes=True, show_layer_names=True)
    history=model.fit(gru_output, y_train.reshape(-1,1,1), epochs = 500, verbose = 1, shuffle = True, callbacks = [es_callback])
    model.save(modelname)
