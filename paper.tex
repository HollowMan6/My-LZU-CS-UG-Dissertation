...
\title{{基于ICEEMDAN和Prophet-GRU-NN}{的短期风速多步组合预测模型}}
...
\entitle{{A Short-Term Wind Speed Multistep-Ahead}{ Hybrid Prediction Model based on}{ ICEEMDAN and Prophet-GRU-NN}}
...
\supervisorsignature{
    % \raisebox{-5pt}{
        \includegraphics[width=70pt]{tutor_sig.pdf}
    % }
}
...
改进自适应噪声完备集合经验模态分解（ICEEMDAN）技术，结合Facebook推出的新型Prophet
时间序列预测框架分析特征，并根据从支持向量回归机（SVR）中获得的灵感，使用基于高斯
径向基函数（RBF）的核主成分分析技术，降低时间序列数据复杂度。此外，借助于
当前最新循环神经网络结构门控循环单元（GRU），采用最前端的高斯误差线性单元（GELU）
激活函数、Nadam优化器、Huber损失函数
应用于神经网络，形成ICEEMDAN-Prophet-GRU模型。最后使用神经网络（NN）整合每个特征
的预测结果，校正风速预测值，提高模型的泛化能力。

选用真实发电场所的气象数据验证模型。结果表明，使用本文提出的多特征
ICEEMDAN-Prophet-GRU-NN模型，提前12小时预测短期风速的MAPE值达到8\%左右
，精度明显提升，因而具有优越性。
...
\begin{figure}[H]
    \centering
	\subfloat[甘肃省范围内]{
        \includegraphics[width=0.5\textwidth]{figures/google_maps.pdf}
    }
	\subfloat[局部范围内]{
        \includegraphics[width=0.5\textwidth]{figures/google_maps_details.pdf}
    }
...
\begin{table}[H]
    ...
    wind & 相关系数 & 显著水平 \\
    \midrule
    lrad & $0.03235^{**}$ & 0.01349 \\
    prec & $0.00788$ & 0.54761 \\
    pres & $0.07469^{**}$ & $1.12504\times10^{-8}$ \\
    shum & $-0.08556^{**}$ & $5.95877\times10^{-11}$ \\
    srad & $0.24626^{**}$ & $2.65041\times10^{-81}$ \\
    temp & $0.05672^{**}$ & $1.46594\times10^{-5}$ \\
    \bottomrule \\
    \end{tabular} \\
    \raggedright
...
\begin{figure}[H]
	\centering
    \includegraphics[width=0.45\textwidth]{figures/tanh.pdf}
    \caption{tanh函数图像}
    \label{fig_tanh}
\end{figure}
...
\begin{figure}[H]
	\centering
    \includegraphics[width=0.45\textwidth]{figures/relu_gelu.pdf}
...
\begin{figure}[H]
	\centering
    \includegraphics[width=0.8\textwidth]{figures/lrad.pdf}
...
\begin{figure}[H]
	\centering
    \includegraphics[width=0.65\textwidth]{figures/shum.pdf}
...
\begin{figure}[H]
	\centering
    \includegraphics[width=0.65\textwidth]{figures/srad.pdf}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/wind.pdf}
...
误差百分比（MAPE）值最小化。最终得到对于风速序列的最优参数
...
\begin{figure}[H]
	\centering
    \includegraphics[width=1\textwidth]{figures/wind_grumodel_plot.pdf}
...
\begin{figure}[H]
	\centering
    \includegraphics[width=0.8\textwidth]{figures/ICEEMDAN-Prophet-GRU-Wind.pdf}
...
\begin{figure}[H]
	\centering
    \includegraphics[width=0.7\textwidth]{figures/all_feature_prophet_gru_nnmodel_plot.pdf}
...
% 下面这些注释掉可以去掉成绩、评语什么的
\supervisorcomment{
    蒋嵩林同学的论文基于目前较为前沿的理论和技术，对这些技术的优劣进行了分析，
    组合了其中表现突出的几种技术，采用了多步预测的方法，选取了多种气象要素特征，
    构建了短期风速预测模型，因而较为新颖。论文还使用了真实风力发电厂的数据验证
    了模型的可靠性，因而具备较高的实际应用价值。此外，论文结构严谨，内容丰富，
    对已有研究的综述和文献参考详实，符合学术规范，总体上完成情况较好。
}

\committeecomment{经答辩小组一致讨论，该论文通过答辩，成绩为良。}
